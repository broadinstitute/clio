...
class AkkaHttpServerDAO private (routes: Route)(
  ...
  override def startup(): Future[Unit] = {
    val bindingFuture = Http().bindAndHandle(
      routes,
      ClioConfig.HttpServer.interface,
      ClioConfig.HttpServer.port
    )
    ...
    bindingFuture onComplete {
      case Success(_) =>
        logger.info(
          ...
          ClioConfig.Version.value,
          ClioConfig.HttpServer.interface,
          Int.box(ClioConfig.HttpServer.port)
        )
  ...
  override def getVersion: Future[String] =
    Future.successful(ClioConfig.Version.value)
  ...
  override def awaitShutdown(): Unit = {
    Await.result(
      ...
      ClioConfig.HttpServer.shutdownTimeout
    )
  }
