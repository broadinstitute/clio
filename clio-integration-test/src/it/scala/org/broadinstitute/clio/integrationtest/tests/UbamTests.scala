...
trait UbamTests { self: BaseIntegrationSpec =>
  ...
  def testWgsUbamLocation(location: Location): Unit = {
    ...
    it should s"handle upserts and queries for wgs-ubam location $location" in {
      for {
        ...
        getByName[Int](storedDocument, "lane") should be(expected.lane)
        getByName[String](storedDocument, "library_name") should be(expected.libraryName)
        getByName[Location](storedDocument, "location") should be(expected.location)
        getByName[String](storedDocument, "project") should be(expected.project.get)
      }
    }
  }
  ...
  it should "assign different upsertIds to different wgs-ubam upserts" in {
    ...
    for {
      ...
      getByName[String](storedDocument1, "project") should be("testProject1")

      val storedDocument2 = getJsonFrom(upsertId2)(ElasticsearchIndex.WgsUbam)
      getByName[String](storedDocument2, "project") should be("testProject2")
  ...
  it should "allow an upsert that modifies values not already set or are unchanged if force is false" in {
    ...
    for {
      ...
      val storedDocument2 = getJsonFrom(upsertId2)(ElasticsearchIndex.WgsUbam)
      getByName[String](storedDocument1, "project") should be("testProject1")
      getByName[String](storedDocument2, "sample_alias") should be("sampleAlias1")
    }
  }
