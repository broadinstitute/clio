...
class LoggingAuditDAO private(implicit ec: ExecutionContext) extends AuditDAO with StrictLogging {
  ...
  override def auditResponse(request: ClioRequest, response: ClioResponse): Future[Unit] = {
    Future {
      logger.info(LoggingAuditDAO.AuditMarker, s"${request.content} -> ${response.content}")
    }
  }

  override def auditException(request: ClioRequest, exception: Exception): Future[Unit] = {
    Future {
      logger.error(LoggingAuditDAO.AuditMarker, s"${request.content} -> $exception", exception)
    }
  }
}
