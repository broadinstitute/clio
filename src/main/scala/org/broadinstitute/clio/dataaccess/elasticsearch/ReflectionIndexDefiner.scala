...
class ReflectionIndexDefiner[A](indexName: String,
                                indexType: String,
                                indexClass: Class[A])
    extends ElasticsearchIndexDefiner[A] {
  override def indexDefinition: ElasticsearchIndex =
    ElasticsearchIndex(indexName, indexType, fieldDefinitions)
...
object ReflectionIndexDefiner {
  private def getFieldDefinition(field: Field) =
    ElasticsearchField(camelToSnake(field.getName), field.getType)
  ...
  private def camelToSnake(value: String): String = {
    CamelRegex.replaceAllIn(
      value,
      matcher => s"_${matcher.group(0).toLowerCase}"
    )
  }
}
